...
1. Unity MCP는 사용하지 않는다.
2. 코드 작성과 테스트 파일 생성은 Codex가 수행한다.
3. 씬/프리팹/Inspector 조작은 Unity 직렬화 파일을 직접 편집하지 않고, 사용자에게 단계별로 안내한다.
4. 코드 수정 후 Unity 에디터에서 `Assets > Refresh` 또는 `Ctrl+R`로 반영하고 `Console`/`Test Runner`로 확인한다.
